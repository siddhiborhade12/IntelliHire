...
def match(data: dict = Body(...)):
    ...
    try:
        ...
        if jd.strip() == "":
            return {"error": "Enter job description"}
        ...
        return {"score": float(round(score * 100, 2))}
...
def ats_score(data: dict = Body(...)):
    ...
    try:
        ...
        skills = [
            "react", "node", "express", "mongodb", "python",
            "java", "sql", "aws", "docker", "kubernetes",
            "machine learning", "deep learning",
            "html", "css", "javascript", "api", "git"
        ]
        ...
        # Skill score (60%)
        skill_score = (match_count / total * 60) if total > 0 else 0

        # Resume length score (20%)
        length_score = 20 if len(resume_text) > 500 else 10

        # Keyword bonus (20%)
        keyword_score = 20 if any(word in resume_lower for word in ["project", "experience", "internship"]) else 10

        final_score = round(skill_score + length_score + keyword_score, 2)
        ...
        if len(resume_text) < 500:
            suggestions.append("Increase resume content (add projects/experience)")

        if "project" not in resume_lower:
            suggestions.append("Add project section")

        if "experience" not in resume_lower:
            suggestions.append("Add experience or internship")

        return {
            "ats_score": final_score,
            "matched_skills": match_count,
            "total_skills": total,
            "suggestions": suggestions if suggestions else ["Your resume is strong!"]
        }
